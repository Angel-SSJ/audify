...
from api.dtos.playback_history import (
    CreatePlaybackHistoryDTO,
    UpdatePlaybackHistoryDTO,
    PlaybackHistoryResponse
)

from api.dependencies.playback_history_dependency import (
    get_playback_histories_service
)

router = APIRouter(
    prefix="/playback-history",
...
async def get_playback_histories(
...
async def create_playback_history(
    history: CreatePlaybackHistoryDTO,
    service: PlaybackHistoryService = Depends(get_playback_histories_service)
):
...
async def restore_playback_history(
    history_id: str,
    service: PlaybackHistoryService = Depends(get_playback_histories_service)
):
...
async def get_cassandra_history(
    user_id: str,
    service: PlaybackHistoryService = Depends(get_playback_histories_service)
):
    return await service.get_user_history_cassandra(user_id)